...
// 타이핑되는 미니 터미널 — 명령 출력으로 자기소개를 보여줌
const TERMINAL_LINES = [
  { cmd: 'whoami', out: '조준범 · Frontend Developer' },
  { cmd: 'cat stack.txt', out: 'React · TypeScript · Tailwind CSS' },
  { cmd: 'ls projects/', out: 'maeumdanjjak  mindroom  newjins  artistry' },
  { cmd: 'echo $LOCATION', out: 'Seoul, Korea' },
  { cmd: 'status --now', out: '● available for work', status: true },
]
...
function TermCursor() {
  return (
    <motion.span
      ...
      className="inline-block w-[7px] h-[14px] -mb-0.5 ml-0.5 align-middle bg-emerald-400/90"
    />
  )
}
...
function Terminal() {
  ...
  useEffect(() => {
    ...
    if (phase === 'typing') {
      if (typing.length < line.cmd.length) {
        const t = setTimeout(() => setTyping(line.cmd.slice(0, typing.length + 1)), 55)
  ...
  const finished = li >= TERMINAL_LINES.length

  return (
    <div className="rounded-xl overflow-hidden border border-black/10 dark:border-white/10 bg-[#0e0e11] shadow-xl shadow-black/20">
      <div className="flex items-center gap-2 px-3.5 py-2.5 bg-white/[0.04] border-b border-white/[0.06]">
        ...
        <span className="ml-2 font-mono text-[11px] text-white/30">junbeom@portfolio: ~</span>
      </div>
      <div className="px-4 py-3.5 font-mono text-[12.5px] leading-relaxed min-h-[190px]">
        {done.map((line, i) => (
          <div key={i} className="mb-1.5">
            ...
            <div className={`pl-4 ${line.status ? 'text-emerald-400' : 'text-white/50'}`}>{line.out}</div>
          </div>
        ))}
        {!finished ? (
          <div className="flex gap-2">
            <span className="text-emerald-400/80 select-none">$</span>
            <span className="text-white/90">{typing}{phase === 'typing' && <TermCursor />}</span>
          </div>
        ) : (
          <div className="flex gap-2">
            <span className="text-emerald-400/80 select-none">$</span>
      ...
      <div className="relative z-10 max-w-6xl mx-auto w-full">
        <div className="grid lg:grid-cols-[1fr_auto] gap-10 items-end">
          ...
          <motion.div
            ...
            className="hidden lg:block w-[360px] pb-2"
          >
            <Terminal />
          </motion.div>
        </div>
        ...
        <motion.div
          ...
          className="lg:hidden mt-8 max-w-md"
        >
          <Terminal />
        </motion.div>
      </div>
